$(document).ready(function() {
    ...
    var eventBlock = $('#event_blocks');
    var rows = [];
    eventBlock.children().each(function( i, value ) {
        var id = value.id;
    ...
    function addBlock(type='', fields,  methodAdd='append', object=null) {
        ...
        const divBlockVideo = eventBlock;
    ...
    var selectorEvent_blocks = $("#event_blocks");
    //sortableRow
    selectorEvent_blocks.sortable({
        handle: '.removeRow',
    ...
    selectorEvent_blocks.disableSelection();
    ...
    function updateDomCSS(rowItem) {
        ...
        var clearfixSelect = $('#event_blocks_'+rowItem+'_clearfix');
    ...
    function showVideo(index) {
        var idVideo = $('#event_blocks_'+index+'_linkVideo').val();
        if (idVideo.length > 0) {
            $('#showVideo_event_blocks_'+index).removeClass( "hide" );
            var src = $('#showVideo_event_blocks_'+index).data('src');
            src += getIdUrlYoutube($('#event_blocks_'+index+'_linkVideo').val());
            $('#showVideo_event_blocks_'+index+' iframe').attr('src', src);
        }
    }
    ...
    function removeFields(index, type, fields) {
        var children = $('#block_event_blocks_'+index).children();
        $('#event_blocks_'+index+'_type').attr('value', type);
        $('#blockName_event_blocks_'+index).text(type);
        $.each(fields, function( i, value ) {
            var elem = $('#'+value+'_event_blocks_'+index)[0];
            $('#'+value+'_event_blocks_'+index).remove();
        });
    ...
    //change langue
    var lang =  $('#event_lang').val();
    var categ =  $('#event_category').val();
    changeCategories(lang, categ);

    $( document ).on( "change", "#event_lang", function() {
    ...
    function changeCategories(lang, categ) {
        $.ajax({
            ...
            success: function (cats) {
                const select = $("#event_category");
                let span = $('#s2id_event_category a span').first();
                ...
                if (categ > 0) {
                    span.text($('#event_category option[value='+categ+']').text());
                } else {
