$(document).ready(function() {
    ...
    var articleBlock = $('#article_blocks');
    var rows = [];
    articleBlock.children().each(function( i, value ) {
        var id = value.id;
        ...
        changeCol('article_blocks_'+index);
    ...
    function updateClearFix() {
        $('.marginBottom').each(function( index ) {
            ...
            $('#clearfix_'+id).remove();
            if (clearfix) {
                $('#'+id).after('<div id="clearfix_'+id+'" class="col-md-12"></div>');
            } else {
    ...
    function addBlock(type='', fields,  methodAdd='append', object=null) {
        ...
        const divBlockVideo = articleBlock;
        ...
        if ($('.select')[0] === undefined) {
            ...
            articleBlock.first().append(html);
        } else {
            var row = $('.select')[0].id;
            const html = tmpl;
            $('#'+row).append(html);
    ...
    var selectorArticle_blocks = $("#article_blocks");
    //sortableRow
    selectorArticle_blocks.sortable({
        handle: '.removeRow',
    ...
    selectorArticle_blocks.disableSelection();
    ...
    function updateDomCSS(rowItem) {
        ...
        var clearfixSelect = $('#article_blocks_'+rowItem+'_clearfix');
    ...
    function updateRowValues() {
        ...
        if (idCms > 0) {
            $.ajax({
                ...
                error: function (err) {
                    alert("An error ocurred while loading attribures rows...");
                }
            });
    ...
    function showVideo(index) {
        var idVideo = $('#article_blocks_'+index+'_linkVideo').val();
        if (idVideo.length > 0) {
            $('#showVideo_article_blocks_'+index).removeClass( "hide" );
            var src = $('#showVideo_article_blocks_'+index).data('src');
            src += getIdUrlYoutube($('#article_blocks_'+index+'_linkVideo').val());
            $('#showVideo_article_blocks_'+index+' iframe').attr('src', src);
        }
    }
    ...
    function changeCol(id){
        ...
        $('#clearfix_block_'+id).remove();
        if (clearfix === true) {
            clearfixClass = separator = 'cms-col-12';
    ...
    function getFields(type) {
        var fields;
        if (type === 'video') {
            fields = ['alt', 'uploadImage', 'content', 'textImage', 'legend', 'linkImage'];
        } else if ( type === 'image') {
            fields = ['content', 'linkVideo', 'width', 'height'];
        } else if (type === 'text') {
            fields = ['linkVideo', 'alt', 'uploadImage', 'width', 'height', 'textImage', 'legend', 'linkImage'];
        }
        return fields;
    }

    function removeFields(index, type, fields) {
        var children = $('#block_article_blocks_'+index).children();
        $('#article_blocks_'+index+'_type').attr('value', type);
        $('#blockName_article_blocks_'+index).text(type);
        $.each(fields, function( i, value ) {
            var elem = $('#'+value+'_article_blocks_'+index)[0];
            $('#'+value+'_article_blocks_'+index).remove();
        });
    ...
    //change langue
    var lang =  $('#article_lang').val();
    var categ =  $('#article_category').val();
    changeCategories(lang, categ);

    $( document ).on( "change", "#article_lang", function() {
    ...
    function changeCategories(lang, categ) {
        $.ajax({
            ...
            success: function (cats) {
                const select = $("#article_category");
                let span = $('#s2id_article_category a span').first();
                ...
                if (categ > 0) {
                    span.text($('#article_category option[value='+categ+']').text());
                } else {
